...
class GPUPoolManager {
  ...
  async getGPUStatus(): Promise<{
    ...
    // Vraie charge des workers GPU (health-checks reels + cache court),
    // pas de valeurs statiques inventees. Import dynamique pour eviter tout
    // cycle d'import avec lib/live-access.
    try {
      const { getGpuWorkersStatus } = await import('@/lib/live-access')
      const cluster = await getGpuWorkersStatus()
      const { totals } = cluster
      const active = totals?.active ?? 0
      const free = totals?.free ?? 0
      const max = totals?.max ?? 0
      const totalLoad = max > 0 ? Math.round((active / max) * 100) : 0
      return {
        freeWorkers: free,
        premiumWorkers: Math.max(0, (totals?.total ?? 0) - (totals?.online ?? 0)),
        totalLoad,
        activeSessions: active,
      }
    } catch (e) {
      console.warn('[v0] GPUPoolManager: getGPUStatus indisponible:', e)
      // Aucune valeur inventee : on reporte 0 plutot que des chiffres faux.
      return { freeWorkers: 0, premiumWorkers: 0, totalLoad: 0, activeSessions: 0 }
    }
  }
}
