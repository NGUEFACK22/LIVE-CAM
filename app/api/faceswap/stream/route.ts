...
export async function POST(request: NextRequest) {
  try {
    ...
    let profilePoints = FREE_UNLIMITED_POINTS;
    ...
    const actualPointsUsed = freeMode
      ? 0
      : Math.ceil((totalProcessingTime / 1000) * POINTS_PER_SECOND);

    if (!freeMode) {
      // Deduct points
      const { error: updateError } = await supabase
        .from("profiles")
        .update({ points: profilePoints - actualPointsUsed })
        .eq("id", user.id);
    ...
    return NextResponse.json({
      ...
      points_used: actualPointsUsed,
      points_remaining: freeMode
        ? FREE_UNLIMITED_POINTS
        : profilePoints - actualPointsUsed,
    });
