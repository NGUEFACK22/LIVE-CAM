...
export function CloudSwapPlayer({
  ...
  const { 
    ...
    userId,
    userTier: userPlan === 'free' ? 'free' : 'premium',
    avatarUrl,
    onMetricsUpdate: (m) => setStats({
      fps: m.fps,
      latency: m.latency,
  ...
  function handleStatsUpdate(newStats: any) {
  ...
  const planLimits: Record<string, { maxFps: number; maxResolution: string; maxMinutes: number }> = {
    free: { maxFps: 15, maxResolution: '480p', maxMinutes: 5 },
    starter: { maxFps: 25, maxResolution: '720p', maxMinutes: 4 },
    standard: { maxFps: 30, maxResolution: '720p', maxMinutes: 20 },
    popular: { maxFps: 30, maxResolution: '1080p', maxMinutes: 50 },
    premium: { maxFps: 30, maxResolution: '1080p', maxMinutes: 100 },
                    ...
                    {hardware?.isGamingPC ? (
                      <div className="space-y-3">
                        ...
                        <div className="p-3 rounded-lg bg-white/5">
                          ...
                          <p className="text-xs text-white/40">{(hardware as any).vram || 8}GB VRAM</p>
                        </div>
                      </div>
                    ) : (
